...
{ 
	s_bind = "s = " + name;
	if (0 != e) {
		port = jack_port_register(e->get_client(), name.c_str(), JACK_DEFAULT_MIDI_TYPE, JackPortIsOutput | JackPortIsTerminal, 0);
	}
}

sequence::~sequence() {
	jdbg("~sequence()")
	jack_port_unregister(e->get_client(), port);
}


void sequence::connect(const std::string &destinationport) {
	jack_connect(e->get_client(), jack_port_name(port), destinationport.c_str());
}
